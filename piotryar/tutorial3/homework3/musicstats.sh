...
# E2.2 (1 point ) If the wget command fails, exit with error.
	if [ $? -ne 0 ];then
		echo -e " Could not download file, wget failed."
...
touch ${STATSDIR}/STIL-recordentries.txt
...
do
   ...
   # the file is stats/Follin_Tim-entries.txt
	...
	touch ${STATSDIR}/${name}-entries.txt
	

	egrep "${name}" ${STATSDIR}/STIL-recordentries.txt >> ${STATSDIR}/${name}-entries.txt
   ...
   #
    awk -F/ '{print $NF}' ${STATSDIR}/${name}-entries.txt >> ${STATSDIR}/${name}-songs.txt
   #
   ### E5.3 END ########################################################
   ...
   # 
	sed -i '/^\r$/d' ${STATSDIR}/${name}-songs.txt
   #
   ### E5.4 END ########################################################
   ...
   #
	NUMSONGS=`wc -l ${STATSDIR}/${name}-songs.txt| cut -f1 -d' '`
   ### E5.5 END ########################################################

    echo "Number of songs for artist $name is $NUMSONGS, storing in ${STATSDIR}/stats.csv" 
   ...
   #
	touch ${STATSDIR}/stats.csv
	echo "${name}","${NUMSONGS}" >> ${STATSDIR}/stats.csv
   #
   ### E5.6 END ########################################################
...
# useful options: -h -k
#
sort -o ${STATSDIR}/stats.csv ${STATSDIR}/stats.csv
chmod 777 ${STATSDIR}/stats.csv
TOPSONGARTIST=$(tail -n 1 ${STATSDIR}/stats.csv)
#
### E6 END #############################################################
