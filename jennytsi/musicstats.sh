...
# The encoding is a ISO-8859 text, with CRLF line terminators
#
...
if [[ -z "$@" ]]; then
	echo "The variable artist name is empty" 
	usage
	exit 1
fi
...
if [[ -d ${STATSDIR} ]]; then
	rm -r ${STATSDIR}
	echo "'stats' folder already exists in the folder, it will be deleted"
fi
