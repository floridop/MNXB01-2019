...
echo "Checking if stats folder already exists"
echo "If it does exists the script will delete it"
if [ -d ${STATSDIR} ]; then
	echo "Deleting existing directory ${STATSDIR}"
	rm -r "${STATSDIR}"
fi
...
for artist in ${ARTISTNAMES}; do
   ...
   grep ${artist} "${STATSDIR}/STIL-recordentries.txt" > "${STATSDIR}/${artist}-entries.txt"
   #
   ### E5.2 END ########################################################
   ...
   # 
   sed -i 's/\r$//' ${STATSDIR}/${artist}-songs.txt
   sed -i '/^$/d' ${STATSDIR}/${artist}-songs.txt
   #
   ### E5.4 END ########################################################
   ...
   #
   NUMSONGS=$(wc -l ${STATSDIR}/${artist}-songs.txt | cut -d " " -f1)
   #
   ### E5.5 END ########################################################
...
TOPSONGARTIST=$(sort -n -k 2 ${STATSDIR}/stats.csv | tail -n1) 
