...
#At first I tried this (back when I did not know thee was a template). 
#I think it is bad programming as a new file needs to be created, but the code
#did not work when I stored it in a variable instead. Can you explain why?
#ls > file.txt
#	STILindir=$(grep -c "STIL.txt" file.txt)
#	if [ "$STILindir" == "0" ]; then
#		echo "downloading STIL.txt"
#		wget http://hvsc.de/download/C64Music/DOCUMENTS/STIL.txt
#		echo "STIL.txt is already downloaded"
#	fi
...
if [[ ! -f ./STIL.txt ]]; then
   ...
   if [[ $? -ne 0 ]]; then
		echo "The data base could not be downloaded"
		exit 1
	fi
...
if [[ -d stats ]]; then
	rm -r ${STATSDIR}
	echo "Old stats directory was deleted"
fi
...
for i in ${ARTISTNAMES}; do
   ...
   #
   egrep ${i} ${STATSDIR}/STIL-recordentries.txt > ${STATSDIR}/${i}-entries.txt
   #
   ### E5.2 END ########################################################
   ...
   #
   awk -F "/" '{print $NF}' ${STATSDIR}/${i}-entries.txt > ${STATSDIR}/${i}-songs.txt
   #
   ### E5.3 END ########################################################
   ...
   # 
   sed -i '/^\r$/d' ${STATSDIR}/${i}-songs.txt
   #
   ### E5.4 END ########################################################
   ...
   #
   NUMSONGS=`wc -l <${STATSDIR}/${i}-songs.txt`
   #
   ### E5.5 END ########################################################
   ...
   #I have no "" around the number, because I could not make the sort 
   #command work on that format
   ### E5.6 END ########################################################
...
# useful options: -h -k
#
TOPSONGARTIST=`sort -t, -k 2n ${STATSDIR}/stats.csv | tail -n 1`
#
### E6 END #############################################################
