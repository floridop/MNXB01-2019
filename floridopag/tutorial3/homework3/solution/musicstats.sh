...
if [[ "x$ARTISTNAMES" == "x" ]]; then
   echo "Error: Missing artist name as input parameter";
   usage
   exit 1
fi
...
# E2.1 (2 points) if the STIL.txt file exists...
if [[ ! -e STIL.txt ]]; then
   echo "STIL database not found. Downloading..."
   wget https://hvsc.de/download/C64Music/DOCUMENTS/STIL.txt
   SUCCESS=$?
   # E2.2 (1 point ) If the wget command fails...
   if [[ $SUCCESS != 0 ]]; then
      echo "Error: wget command failed, cannot continue."
      exit 1
   fi
...
if [[ -e $STATSDIR ]]; then
   echo "directory $STATSDIR found, removing old one"
   rm -rf $STATSDIR
fi
...
# body
for ARTISTNAME in $ARTISTNAMES; do
   ...
   NUMSONGS=$(wc -l ${STATSDIR}/${ARTISTNAME}-songs.txt | cut -d' ' -f1)
   ### E5.5 END ########################################################
...
# useful options: -h -k
# Note the trick of using \" as separator. This forces sort to interpret
# the number of songs as a number instead of a string.
TOPSONGARTIST=$(sort -t\" -h -k4 ${STATSDIR}/stats.csv | tail -1)
### E6 END #############################################################
