...
if [[ -n "$ARTISTNAMES" ]]; then
   ...
   echo "There is no artist name argument at the start of the script"
   usage
   exit 1
fi
...
if [[ -e $STATSDIR ]]; then
   echo "old stats directory found deleting"
   rm -r $STATSDIR
fi
...
# useful options: -h -k
#
TOPSONGARTIST=$(sort -t, -k 2 ${STATSDIR}/stats.csv | tail -n2 | head -n1)
#
### E6 END #############################################################
