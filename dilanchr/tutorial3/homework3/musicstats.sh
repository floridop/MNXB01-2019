...
if [[ -z "$ARTISTNAMES" ]]; then
	usage
	echo "There is no artist matching the input argument"
	exit 1
fi
...
if [ -d ./"$STATSDIR" ]; then
	echo "Removing old stats directory and its contents"
	rm -r "$STATSDIR"
fi
...
RENTRIES=STIL-recordentries.txt
egrep -n '^/' ./"$FILE" > ${STATSDIR}/${RENTRIES}
...
for arg in "$@"; do
   ...
   #YOUR_CODE_HERE
	ARTISTNAME=$arg
	grep "$arg" ./${STATSDIR}/${RENTRIES} > ./${STATSDIR}/${ARTISTNAME}-entries.txt
   ...
   #
   #YOUR_CODE_HERE
	NUMSONGS=$(wc -l ./${STATSDIR}/${ARTISTNAME}-songs.txt | cut -f1 -d' ')   
   #
   ### E5.5 END ########################################################
...
# useful options: -h -k
#
TOPSONGARTIST=$(sort -h ./${STATSDIR}/stats.csv | tail -n1)
#
### E6 END #############################################################
