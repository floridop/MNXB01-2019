...
if [[ -z "$@" ]]; then
	echo "The variable $ARTISTNAMES is empty"
	usage
	exit 1
fi
...
if [[ -d ${STATSDIR} ]]; then 
	rm -r stats
	echo "The existing stats folder has been deleted."
fi
...
#                
#
egrep -n ^/ STIL.txt > stats/STIL-recordentries.txt
...
for ARTISTNAME in ${ARTISTNAMES}
   ...
   #
		grep ${ARTISTNAME} ${STATSDIR}/STIL-recordentries.txt > stats/${ARTISTNAME}-entries.txt
   #
   ### E5.2 END ########################################################
   ...
   # 
   sed -i '/^\r$/d' stats/${ARTISTNAME}-songs.txt
   #
   ### E5.4 END ########################################################
