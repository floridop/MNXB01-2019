...
# The encoding is UTF-8 Unicode text.____________________ 
#
...
if [ ! -e STIL.txt ]; then 
   ...
   if [["$?" != 0]] ;then
      echo "Fail downloading the file"
	  exit 1
...
if [[ -e ./$STATSDIR ]]; then
	rm -r $STATSDIR/
	echo "The temporary folder $STATSDIR is deleted"
fi
...
for artist in "$@"; do
   ...
   #
   NUMSONGS=`wc -l ${STATSDIR}/${artist}-songs.txt | cut -f1 -d' '`
   #
   ### E5.5 END ########################################################
