...
if [[-z ARTISTNAMES]]; then echo -e "ARTISTNAMES is empty"
usage(){
	echo "----"
	echo -e "  Wrong syntax. To call this script please use"
	echo -e "   $0 '<artistlastname>_<artistname>' ['<artistlastname>_<artistname>' ...]"
...
if [-f "STIL.txt"]; then wget https://hvsc.de/download/C64Music/DOCUMENTS/STIL.txt
...
if [-d $STATSDIR]; then rm $STATSDIR 
echo "Removing Stats"
...
#                
#
egrep -n ^/ > {STATSDIR}/STIL-recordentries.txt 
...
# body
for $ARTISTNAMES in STIL-recordentries.txt
   ...
   #
   egrep -n ^$ARTISTNAMES > stats/$ARTISTNAMES-entries.txt
   #
   ### E5.2 END ########################################################
   ...
   #
   #YOUR_CODE_HERE
   egrep -n $ARTISTNAMES / $NF > ${STATSDIR}/${ARTISTNAME}-songs.txt
   #
   ### E5.3 END ########################################################
   ...
   # 
   sed '^\r/d' ${STATSDIR}/${ARTISTNAME}-songs.txt 
   #
   ### E5.4 END ########################################################
   ...
   #
   NUMSONGS=wc -l < ${STATSDIR}/${ARTISTNAME}-songs.txt
   #
   ### E5.5 END ########################################################
   ...
   #
   echo "\$ARTISTNAMES", "\$NUMSONGS!" >> ${STATSDIR}/stats.csv.
   #
   ### E5.6 END ########################################################
...
# useful options: -h -k
#
sort ${STATSDIR}/stats.csv > ${SORTED}/stats.csv
TOPSONGARTIST= tail ${SORTED}/stats.csv
#
### E6 END #############################################################
