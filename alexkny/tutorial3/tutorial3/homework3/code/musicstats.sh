...
if [ -e $STATSDIR ]; then
    echo "$STATSDIR directory found, deleting..."
    rm -vrf $STATSDIR
fi
...
#                
#
egrep -n ^/ $DATABASE > ${STATSDIR}/STIL-recordentries.txt
#
### E4 END #############################################################
...
for ((i=0; i<${#ARTISTNAMES[@]}; i++)); do
   ...
   #
   ARTIST_ENTRIES="${ARTISTNAMES[$i]}-entries.txt"
   egrep -i ${ARTISTNAMES[$i]} ${STATSDIR}/STIL-recordentries.txt > ${STATSDIR}/${ARTIST_ENTRIES}
   #
   ### E5.2 END ########################################################
   ...
   #
   ARTIST_SONGS="${ARTISTNAMES[$i]}-songs.txt"
   awk -F "/" '{print $NF}' ${STATSDIR}/${ARTIST_ENTRIES} > ${STATSDIR}/${ARTIST_SONGS}
   #
   ### E5.3 END ########################################################
   ...
   # 
   sed -i '/^\r$/d' ${STATSDIR}/${ARTIST_SONGS}
   #
   ### E5.4 END ########################################################
   ...
   NUMSONGS=$( wc -l ${STATSDIR}/${ARTIST_SONGS} | cut -d " " -f1 )
   #
   ### E5.5 END ########################################################
...
# useful options: -h -k
#
TOPSONGARTIST=$( sort -nt "," -k 2.2 ${STATSDIR}/stats.csv | tail -1 | cut -d "," -f1 | sed 's/.$//;s/^.//' )
#
### E6 END #############################################################
