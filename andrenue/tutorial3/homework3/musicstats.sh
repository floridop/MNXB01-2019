...
if test -z "$ARTISTNAMES"
then
   echo "\$ARTSTNAMES is empty"
   usage
fi
########################################################################
...
if [ -d "$STATSDIR" ]; then
  ### Take action if $STATSDIR exists ###
  echo " the '$STATSDIR' folder exists, it will be deleted"
  rm -rf $STATSDIR
else
  ###  Control will jump here if $STATSDIR does NOT exists ###
  echo "The '$STATSDIR' folder is not found."
fi
...
#
grep '^/' STIL.txt > ${STATSDIR}/STIL-recordentries.txt
#
### E4 END #############################################################

########################################################################
### E5 (14 points)  - use a for loop to cycle the artist names contained in the
...
for i in ${ARTISTNAMES}; do
   ...
   #
	grep ${i} ${STATSDIR}/STIL-recordentries.txt> ${STATSDIR}/${i}-entries.txt
   ### E5.2 END ########################################################
   ...
   echo "Number of songs for artist $i is $NUMSONGS, storing in ${STATSDIR}/stats.csv" 
