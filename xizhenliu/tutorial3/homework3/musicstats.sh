...
if [ -d $STATSDIR ]; then
   echo "Directory $STATSDIR exist. Remove it..."
   rm -r $STATSDIR
fi
...
do
   ...
   #
   cat ${STATSDIR}/STIL-recordentries.txt | grep ${name} > ${STATSDIR}/${name}-entries.txt
   #
   ### E5.2 END ########################################################
   ...
   #
   NUMSONGS=$(wc -l ${STATSDIR}/${name}-songs.txt | sed -e 's/^[[:space:]]*//' | cut -d ' ' -f 1)
   #
   ### E5.5 END ########################################################
...
# useful options: -h -k
#
TOPSONGARTIST=$(sort -h -k 2 ${STATSDIR}/stats.csv | tail -n 1)
#
### E6 END #############################################################
