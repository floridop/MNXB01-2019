...
if [ ! -e $FILEPATH ]; then
	wget -O STIL.txt "https://hvsc.de/download/C64Music/DOCUMENTS/STIL.txt"
	#echo "no file exists"
...
if [ -e $STATSDIR ]; then
	echo "$STATSDIR folder already exists, deleting old version"
	rm -r "$STATSDIR" 
fi
...
#                
#
cat "$FILEPATH" | egrep -n "^/" > ${STATSDIR}/STIL-recordentries.txt 
#
### E4 END #############################################################
...
do
   ...
   #
   cat ${STATSDIR}/STIL-recordentries.txt | grep ${ARTISTNAME}  > ${STATSDIR}/${ARTISTNAME}-entries.txt
   #
   ### E5.2 END ########################################################
   ...
   # 
   sed -i '/^$/d' ${STATSDIR}/${ARTISTNAME}-songs.txt | sed -i '/^\r$/d' ${STATSDIR}/${ARTISTNAME}-songs.txt
   #
   ### E5.4 END ########################################################
...
# useful options: -h -k
#
TOPSONGARTIST=$(sort -t '"' -k4 -h ${STATSDIR}/stats.csv | tail -1)
#
### E6 END #############################################################
