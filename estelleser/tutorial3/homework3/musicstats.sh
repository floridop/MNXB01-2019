...
if [[ -z $ARTISTNAMES ]]; then
	echo "No parameter given. Please give parameter"
	usage
	exit 1
fi
#echo "OK"
...
if [[ -e $STATSDIR ]]; then
    echo "Target folder already exists. Deleting..."
    rm -r $STATSDIR
fi
...
# useful options: -h -k
#
TOPSONGARTIST=`sort -t '"' -k 4 -n ${STATSDIR}/stats.csv | tail -n 1`
#
### E6 END #############################################################
