...
FILE=~/homework3/code/samplefolder/STIL.txt
if [[ ! -e $FILE ]]; then
	echo "Files does not exist we will download it"
	WGET=$(wget https://hvsc.de/download/C64Music/DOCUMENTS/STIL.txt -P ~/homework3/code/samplefolder)
...
RM=$(rm -r ~/homework3/code/stats)

# E3.3 (1 point ) Create a new folder taking the name from the 
# variable $STATSDIR
CR=$(mkdir ~/homework3/code/$STATSDIR)
else
CR=$(mkdir ~/homework3/code/$STATSDIR)
fi
...
EGRP=$(egrep -n '^/' ~/homework3/code/samplefolder/STIL.txt>${STATSDIR}/STIL-recordentries.txt)
...
echo "Artist number $i is ${!i}";
   ...
   #
EGRP_song=$(egrep -n "${!i}" ~/homework3/code/stats/STIL-recordentries.txt>${STATSDIR}/${!i}-entries.txt)
...
echo "deleting all blank line in ${!i}-song.txt"
   ...
   #
ARTISTNAME=${!i}
NUMSONGS=$(wc ~/homework3/code/${STATSDIR}/${!i}-songs.txt | cut -d " " -f 2-3)
   ### E5.5 END ########################################################
...
# useful options: -h -k
#
TOPSONGARTIST=$(sort -nr ${STATSDIR}/stats.csv>${STATSDIR}/stats.csv | tail -n 1  ${STATSDIR}/stats.csv)
